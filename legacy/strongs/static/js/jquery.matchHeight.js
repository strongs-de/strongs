...
(function ($) {
	...
	$.fn.matchHeight._apply = function (elements, byRow, variant2) {
		...
		if (byRow) {
			...
			if (variant2) rows = _rows2($elements);
			else rows = _rows($elements);
		...
		$.each(rows, function (key, row) {
			...
			$row.each(function () {
				...
				if ($that.css('box-sizing') !== 'border-box') {
					...
					verticalPadding +=
						parseInt($that.css('padding-top'), 10) + parseInt($that.css('padding-bottom'), 10);
				}
				...
				if ($that.offset().top < yPos) $that.css('margin-top', yPos - $that.offset().top);
			});
		});
		...
		$elements.each(function () {
			...
			if (lastRow === null && $that.css('visibility') != 'hidden') {
				...
				for (i = 0; i < rows.length; ++i) {
					if (
						Math.floor(
							Math.abs(rows[i].offset().top - parseInt($that.css('margin-top'), 10) - top)
						) <= tolerance
					) {
						rows[i] = rows[i].add($that);
						added = true;
