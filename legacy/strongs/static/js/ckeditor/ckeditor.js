...
(function () {
						...
						function b() {
							try {
								document.addEventListener
									? (document.removeEventListener('DOMContentLoaded', b, !1), e())
	...
	CKEDITOR.env ||
		(CKEDITOR.env = (function () {
			...
			b.cssClass =
				'cke_browser_' +
				(b.ie ? 'ie' : b.gecko ? 'gecko' : b.opera ? 'opera' : b.webkit ? 'webkit' : 'unknown');
			if (b.quirks) b.cssClass = b.cssClass + ' cke_browser_quirks';
			if (b.ie) {
				b.cssClass =
					b.cssClass + (' cke_browser_ie' + (b.quirks || b.version < 7 ? '6' : b.version));
		...
		CKEDITOR.tools = {
			...
			extend: function (b) {
				...
				for (var d = 1; d < a; d++) {
					var l = arguments[d],
						h;
					for (h in l) if (e === true || b[h] == void 0) if (!i || h in i) b[h] = l[h];
				}
				return b;
			},
				...
				for (var b = [].concat(b), a, e = [], i = 0; i < b.length; i++)
					if ((a = b[i]))
						/@import|[{}]/.test(a)
							...
							: e.push('<link type="text/css" rel=stylesheet href="' + a + '">');
				return e.join('');
			},
			...
			setTimeout: function (b, a, e, i, d) {
				...
				return d.setTimeout(function () {
					i ? b.apply(e, [].concat(i)) : b.apply(e);
				}, a || 0);
			},
			trim: (function () {
				var b = /(?:^[ \t\n\r]+)|(?:[ \t\n\r]+$)/g;
			...
			removeAllListeners: function () {
				...
				for (b in a) {
					...
					this.$.detachEvent
						...
						: this.$.removeEventListener && this.$.removeEventListener(b, c, false);
					delete a[b];
				}
	...
	CKEDITOR.tools.extend(CKEDITOR.dom.node.prototype, {
		...
		getPosition: function (a) {
			...
			for (
				var d = this.getAddress(), a = a.getAddress(), b = Math.min(d.length, a.length), c = 0;
				c <= b - 1;
				c++
			)
				if (d[c] != a[c]) {
				...
				for (var a = this.getChildren(), b = 0, c = a.count(); b < c; b++) {
					...
					if (
						...
						((d.type == CKEDITOR.NODE_ELEMENT && !d.isEmptyInlineRemoveable()) ||
							(d.type == CKEDITOR.NODE_TEXT && CKEDITOR.tools.trim(d.getText())))
					)
						return false;
				}
				return true;
			...
			removeStyle: function (a) {
				...
				if (!b.removeProperty && (a == 'border' || a == 'margin' || a == 'padding')) {
					...
					for (var b = [], h = 0; h < c.length; h++)
						if (d) for (var k = 0; k < d.length; k++) b.push([a, c[h], d[k]].join('-'));
	...
	(function () {
					...
					f.type == CKEDITOR.NODE_ELEMENT &&
						(f =
							d > 0
								...
								: m(f, true) === false
									? null
									: f.getPreviousSourceNode(true, n, m));
				else {
					f = e;
		...
		CKEDITOR.dom.walker.bogus = function (a) {
			...
			return function (c) {
				...
				if (f) {
					...
					c = c.getNext(b);
					f =
						f.isBlockBoundary() && (!c || (c.type == CKEDITOR.NODE_ELEMENT && c.isBlockBoundary()));
				}
				return !!(a ^ f);
			};
		};
		...
		CKEDITOR.dom.element.prototype.getBogus = function () {
			...
			return a &&
				(CKEDITOR.env.needsBrFiller ? a.is && a.is('br') : a.getText && i.test(a.getText()))
				? a
				: false;
		};
	})();
	...
	(function () {
		function a() {
			...
			return function (f) {
				...
				return (f.type == CKEDITOR.NODE_TEXT &&
					(f.hasAscendant('pre') || CKEDITOR.tools.trim(f.getText()).length)) ||
					(f.type == CKEDITOR.NODE_ELEMENT && !f.is(i))
					? false
					: true;
			};
		}
		function d(a) {
			...
			return function (e) {
				return c(e) || b(e)
					? true
					: (!a && g(e)) || (e.type == CKEDITOR.NODE_ELEMENT && e.is(CKEDITOR.dtd.$removeEmpty));
			};
		}
		function b(a) {
			return function () {
					...
					if (!i[c] || k.$.parentNode != i[c].$.parentNode)
						for (k = k.getPrevious(); k;) {
							...
							if (b == 2) t.$.insertBefore(k.$.cloneNode(true), t.$.firstChild);
								...
								k.remove();
								b == 1 && t.$.insertBefore(k.$, t.$.firstChild);
							}
							k = l;
						}
					t && (t = G);
		...
		CKEDITOR.dom.range.prototype = {
			...
			createBookmark2: (function () {
				function a(b) {
					...
					var d = e;
					f =
						f.type != CKEDITOR.NODE_ELEMENT || d === 0 || d == f.getChildCount()
							? 0
							: f.getChild(d - 1).type == CKEDITOR.NODE_TEXT &&
								f.getChild(d).type == CKEDITOR.NODE_TEXT;
					if (f) {
						c = c.getChild(e - 1);
						e = c.getLength();
					}
				...
				switch (a) {
					...
					case CKEDITOR.ENLARGE_ELEMENT:
						...
						if (t.type == CKEDITOR.NODE_TEXT) {
							if (l) {
								t = !CKEDITOR.tools.trim(t.substring(0, l)).length && t;
								h = !!t;
							}
							...
							for (; n;) {
								...
								if (n.type == CKEDITOR.NODE_COMMENT) n = n.getPrevious();
									...
									if (n.type == CKEDITOR.NODE_TEXT) {
										...
										e.test(g) && (n = null);
										t = /[\s\ufeff]$/.test(g);
									} else if ((n.$.offsetWidth > 0 || (b && n.is('br'))) && !n.data('cke-bookmark'))
					...
					case CKEDITOR.ENLARGE_LIST_ITEM_CONTENTS:
						...
						this.setStartAt(
							H,
							!H.is('br') && ((!j && this.checkStartOfBlock()) || (j && H.contains(j)))
						...
						this.setEndAt(
							H,
							(!j && this.checkEndOfBlock()) || (j && H.contains(j))
				...
				if (!this.collapsed) {
					...
					e.guard = function (b, f) {
						...
						if (
							...
							(f && b.equals(r)) ||
							(c === false && b.type == CKEDITOR.NODE_ELEMENT && b.isBlockBoundary()) ||
					...
					if (m)
						(f = e[a == CKEDITOR.SHRINK_ELEMENT ? 'lastForward' : 'next']()) &&
							this.setStartAt(
								f,
					...
					if (j) {
						e.reset();
						(e = e[a == CKEDITOR.SHRINK_ELEMENT ? 'lastBackward' : 'previous']()) &&
							this.setEndAt(e, b ? CKEDITOR.POSITION_BEFORE_END : CKEDITOR.POSITION_AFTER_END);
					}
					return !(!m && !j);
				}
			},
				...
				function a(b, c) {
					for (; b;) {
						if (b.type == CKEDITOR.NODE_ELEMENT) {
							...
							if (
								b.is('html') ||
								(b.getAttribute('contentEditable') == 'true' && (b.contains(c) || b.equals(c)))
							)
								break;
						}
						b = b.getParent();
				...
				if (a.is(CKEDITOR.dtd.$block)) {
					if ((f = c[b ? 'getNextEditableNode' : 'getPreviousEditableNode']())) {
						...
						if (
							...
							f.getAttribute('contenteditable') == 'false'
						) {
		...
		a.prototype = {
			getNextParagraph: function (a) {
				...
				if (!this._.started) {
					...
					g =
						f.endContainer.hasAscendant('pre', true) || f.startContainer.hasAscendant('pre', true);
					f.enlarge(
						(this.forceBrBreak && !g) || !this.enlargeBr
				...
				for (this._.nextNode = null; p;) {
					...
					if (m) p.type == CKEDITOR.NODE_TEXT && e.test(p.getText()) && (m = 0);
						...
						if (CKEDITOR.dtd.$block[n] && p.getAttribute('contenteditable') == 'false') {
							...
							if (n == 'br') m = 1;
							else if (!g && !p.getChildCount() && n != 'hr') {
								b = p;
								s = p.equals(f);
								break;
							}
				...
				if (q)
					(q = b.getPrevious()) &&
						...
						(q.getName() == 'br'
							? q.remove()
							: q.getLast() &&
								q.getLast().$.nodeName.toLowerCase() == 'br' &&
								q.getLast().remove());
				if (u)
					(q = b.getLast()) &&
						q.type == CKEDITOR.NODE_ELEMENT &&
						q.getName() == 'br' &&
						(!CKEDITOR.env.needsBrFiller || q.getPrevious(i) || q.getNext(i)) &&
						q.remove();
	...
	(function () {
				...
				if (b == 'br') {
					...
					b: {
						...
						for (var d = 0, j = e.length, i; d < j; ++d) {
							i = e[d];
							if (i.type == CKEDITOR.NODE_ELEMENT && !f[i.name]) {
								f = false;
								break b;
							}
						}
					...
					if (f) {
						...
						f = a.parent;
						for (
							var d = f.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT || f.name == 'body',
								m,
								j = e.length;
							j > 0;
						) {
							...
							if (
								...
								(i.type == CKEDITOR.NODE_TEXT ||
									(i.type == CKEDITOR.NODE_ELEMENT && n.$inline[i.name]))
							) {
								if (!m) {
		...
		function y(a, b) {
			...
			for (d in e)
				if (d == 'class') {
					c = e[d].split(/\s+/);
					for (i = a.classes.join('|'); (m = c.pop());) if (i.indexOf(m) == -1) return false;
		...
		CKEDITOR.filter.prototype = {
			...
			applyTo: function (a, b, e, d) {
					...
					function (a) {
						if (a.type == CKEDITOR.NODE_ELEMENT) {
							...
							a: {
								...
								e.children.length == 1 &&
									(f = e.children[0]).type == CKEDITOR.NODE_ELEMENT &&
									p(f, i, m, c, b);
								c = !c.length;
							}
							c || j.push(a);
				...
				j.length && (t = true);
				for (
					var r, a = [], d = w[d || (this.editor ? this.editor.enterMode : CKEDITOR.ENTER_P)];
					(e = j.pop());
				)
			...
			check: function (a, b, e) {
				...
				b =
					n.length > 0
						? false
						: CKEDITOR.tools.objectCompare(d.attributes, m.attributes, true)
							? true
							: false;
			...
			H = (CKEDITOR.filter.transformationsTools = {
				...
				lengthToAttribute: function (a, b, c) {
					...
					if (!(c in a.attributes)) {
						var e = a.styles[b],
							f = e && e.match(/^(\d+)(?:\.\d*)?px$/);
						f ? (a.attributes[c] = f[1]) : e == t && (a.attributes[c] = t);
					}
					delete a.styles[b];
				},
	...
	CKEDITOR.resourceManager.prototype = {
		...
		load: function (a, d, b) {
			...
			for (
				var c = this.loaded, e = this.registered, i = [], g = {}, l = {}, h = 0;
				h < a.length;
				h++
			) {
				var k = a[h];
									...
									if (s.icons)
										for (var u = s.icons.split(','), f = u.length; f--;)
											CKEDITOR.skin.addIcon(
												...
												s.path +
													'icons/' +
													(CKEDITOR.env.hidpi && s.hidpi ? 'hidpi/' : '') +
													u[f] +
													'.png'
											);
									d[l] = 1;
	...
	(function () {
		function a(a, c, i) {
			...
			if (c !== void 0) {
				...
				if (
					!(i == CKEDITOR.ELEMENT_MODE_INLINE
					...
					throw Error(
						'The specified element mode is not supported on element: "' + c.getName() + '".'
					);
				this.element = c;
				this.elementMode = i;
				this.name =
					this.elementMode != CKEDITOR.ELEMENT_MODE_APPENDTO && (c.getId() || c.getNameAtt());
			...
			a.on('customConfigLoaded', function () {
				...
				a.readOnly = !(
					...
					!(a.elementMode == CKEDITOR.ELEMENT_MODE_INLINE
						...
						: a.elementMode == CKEDITOR.ELEMENT_MODE_REPLACE && a.element.hasAttribute('disabled'))
				);
				a.blockless =
					a.elementMode == CKEDITOR.ELEMENT_MODE_INLINE
		...
		function o(a) {
			var b = a.config,
				...
				d = b.removePlugins;
			if (e)
				var j = RegExp('(?:^|,)(?:' + e.replace(/\s*,\s*/g, '|') + ')(?=,|$)', 'g'),
					c = c.replace(j, ''),
					c = c + (',' + e);
			if (d)
				var i = RegExp('(?:^|,)(?:' + d.replace(/\s*,\s*/g, '|') + ')(?=,|$)', 'g'),
					c = c.replace(i, '');
				...
				for (var m in c) {
					...
					if (h && !a.lang[m]) {
						...
						if (CKEDITOR.tools.indexOf(h, a.langCode) >= 0) k = a.langCode;
							...
							k =
								k != a.langCode && CKEDITOR.tools.indexOf(h, k) >= 0
									? k
									: CKEDITOR.tools.indexOf(h, 'en') >= 0
										? 'en'
										: h[0];
						}
		...
		CKEDITOR.tools.extend(CKEDITOR.editor.prototype, {
			...
			execCommand: function (a, b) {
				...
				if (
					...
					this.fire('beforeCommandExec', e) !== false
				) {
	...
	(function () {
		function a(a) {
			return a.attributes['data-cke-survive']
				? false
				: (a.name == 'a' && a.attributes.href) || CKEDITOR.dtd.$removeEmpty[a.name];
		}
		...
		CKEDITOR.htmlParser.fragment.fromHtml = function (i, g, l) {
			function h(a) {
				var b;
				if (y.length > 0)
			...
			function q(a, b) {
				if ((a == p || a.name == 'body') && l && (!a.name || CKEDITOR.dtd[a.name][l])) {
					var c, e;
					return (
						((c = b.attributes && (e = b.attributes['data-cke-real-element-type']) ? e : b.name) &&
			...
			var f = new CKEDITOR.htmlParser(),
				p =
					g instanceof CKEDITOR.htmlParser.element
						? g
						: typeof g == 'string'
							? new CKEDITOR.htmlParser.element(g)
			...
			f.onTagOpen = function (e, i, g, p) {
				...
				if (i.isUnknown && g) i.isEmpty = true;
				i.isOptionalClose = p;
				if (a(i)) y.push(i);
				else {
					if (e == 'pre') n = true;
					...
					if (e == 'br') A.push(i);
						...
						for (;;) {
							p = (g = m.name)
								? CKEDITOR.dtd[g] || (m._.isBlockLike ? CKEDITOR.dtd.div : CKEDITOR.dtd.span)
								: c;
							if (!i.isUnknown && !m.isUnknown && !p[e])
								if (m.isOptionalClose) f.onTagClose(g);
									...
									((g = g[g.length - 1]) && g.name == 'li') ||
										s((g = new CKEDITOR.htmlParser.element('li')), m);
		...
		CKEDITOR.htmlParser.filter = CKEDITOR.tools.createClass({
			...
			proto: {
				...
				onElement: function (a, c) {
					for (
						var e = [this.elementsRules['^'], this.elementsRules[c.name], this.elementsRules.$],
							d,
							g = 0;
						g < 3;
						g++
		...
		a.prototype = {
			...
			exec: function (a, c) {
				...
				for (s = 0; s < l; s++) {
					...
					o = g[s];
					if (!(
						(a.nonEditable && !o.options.applyToAll) ||
			...
			execOnName: function (a, c) {
				for (var e = 0, d = this.rules, g = d.length, l; c && e < g; e++) {
					l = d[e];
					!(
						(a.nonEditable && !l.options.applyToAll) ||
						(a.nestedEditable && l.options.excludeNestedEditable)
					) && (c = c.replace(l.value[0], l.value[1]));
				}
				return c;
			}
		};
	...
	(function () {
			...
			function j(a, e) {
				return function (d) {
					if (d.type != CKEDITOR.NODE_DOCUMENT_FRAGMENT) {
						...
						if ((j = (CKEDITOR.env.opera && !a) || (typeof e == 'function' ? e(d) !== false : e)))
		...
		function i(a) {
			return (
				a &&
				((a.type == CKEDITOR.NODE_ELEMENT && a.name in n) ||
		...
		function k(a) {
			return a.replace(G, function (a, b, c) {
				return (
					'<' +
					b +
					c.replace(I, function (a, b) {
						return B.test(b) && c.indexOf('data-cke-saved-' + b) == -1
		...
		function o(a, b) {
			return a.replace(b, function (a, b, c) {
				a.indexOf('<textarea') === 0 &&
					(a = b + u(c).replace(/</g, '&lt;').replace(/>/g, '&gt;') + '</textarea>');
		...
		function p(a, b) {
			for (
				var c = [],
					...
					e = [/<script[\s\S]*?<\/script>/gi, /<noscript[\s\S]*?<\/noscript>/gi].concat(e),
			...
			b.on(
				...
				function (a) {
					...
					a.dataValue = CKEDITOR.htmlParser.fragment.fromHtml(
						c,
						a.context,
						a.fixForBody === false ? false : d(a.enterMode, b.config.autoParagraph)
					);
				},
				null,
				null,
					...
					table: function (a) {
						a.children.slice(0).sort(function (a, b) {
							...
							if (a.type == CKEDITOR.NODE_ELEMENT && b.type == a.type) {
								...
								e = CKEDITOR.tools.indexOf(j, b.name);
							}
							if (!(c > -1 && e > -1 && c != e)) {
	...
	(function () {
		function a(a) {
			return function (b) {
				return (
					b.type == CKEDITOR.NODE_ELEMENT && (typeof a == 'string' ? b.name == a : b.name in a)
				);
			};
		}
		var d = function (a, b) {
			...
			new CKEDITOR.htmlParser.node(),
			{
				...
				filter: function (a, b) {
					...
					for (h in g) {
						...
						k && ((l = a.onAttribute(b, d, k, l)) === false ? delete g[k] : (g[k] = l));
					}
				...
				setHtml: function (a) {
					for (
						var a = (this.children = CKEDITOR.htmlParser.fragment.fromHtml(a).children),
							b = 0,
							d = a.length;
						b < d;
						++b
				...
				split: function (a) {
					for (
						var b = this.children.splice(a, this.children.length - a), d = this.clone(), g = 0;
						g < b.length;
						++g
					)
						b[g].parent = d;
				...
				removeClass: function (a) {
					...
					if (b)
						(b = CKEDITOR.tools.trim(b.replace(RegExp('(?:\\s+|^)' + a + '(?:\\s+|$)'), ' ')))
				...
				getFilterContext: function (a) {
					...
					!a.nonEditable && this.attributes.contenteditable == 'false'
	...
	(function () {
		...
		CKEDITOR.template = function (i) {
			if (a[i]) this.output = a[i];
				...
				this.output = a[i] = Function(
					'data',
					'buffer',
					"return buffer?buffer.push('" + g + "'):['" + g + "'].join('');"
				);
			}
		};
	})();
	...
	(function () {
		...
		CKEDITOR.editor.prototype.resize = function (a, b, c, d) {
			...
			k.setStyle(
				'height',
				Math.max(b - (c ? 0 : (d.$.offsetHeight || 0) - (k.$.clientHeight || 0)), 0) + 'px'
			);
			o && (o.style.width = '100%');
			this.fire('resize');
		};
	...
	(function () {
		function a(a) {
			...
			if (
				...
				!e.block &&
				j.collapsed &&
				!j.getCommonAncestor().isReadOnly()
			) {
				e = j.clone();
						...
						this.attachListener(a, 'key', function (b) {
							...
							if (c in m) {
								var b = a.getSelection(),
									...
									c = c == 8;
								if (
									(CKEDITOR.env.ie && CKEDITOR.env.version < 11 && (e = b.getSelectedElement())) ||
									(e = i(b))
								) {
									a.fire('saveSnapshot');
									...
									if (
										...
										(p = l[c ? 'getPrevious' : 'getNext'](o))
									) {
										a.fire('saveSnapshot');
										g['moveToElementEdit' + (c ? 'End' : 'Start')](p);
										g.checkStartOfBlock() && g.checkEndOfBlock() ? p.remove() : g.select();
										a.fire('saveSnapshot');
										d = 1;
									} else if (
		...
		var q = (function () {
				...
				function b(c, d) {
					...
					for (
						...
						} else g.push({ isElement: 0, node: e, allowed: 1 });
					}
				...
				function m(b, c, d, e) {
					...
					if (
						(g = new CKEDITOR.dom.walker(j).next()) &&
						a(g) &&
						i[g.getName()] &&
						(n = g.getPrevious()) &&
						a(n) &&
						!n.getParent().equals(b.startContainer) &&
						d.contains(n) &&
						e.contains(g) &&
						g.isIdentical(n)
					) {
				...
				var n = CKEDITOR.dtd,
					...
					g = { p: 1, div: 1, h1: 1, h2: 1, h3: 1, h4: 1, h5: 1, h6: 1 },
				...
				return function (i, h, k) {
					...
					if (!o.checkReadOnly()) {
						...
						for (
							;
							(z = a(r.startContainer) && r.startContainer.getChild(r.startOffset - 1)) &&
							a(z) &&
							z.isBlockBoundary() &&
							C.contains(z);
						)
						...
						if ((x = k)) {
							...
							if (/^\s+|\s+$/.test(k) && 'span' in CKEDITOR.dtd[z])
								var s = '<span data-cke-marker="1">&nbsp;</span>',
									k = s + k + s;
						...
						if (x) {
							...
							for (j(C, E); x < C.length; x++) {
									...
									if (D.hasBlockSibling) u = 1;
										...
										if (!M || !M.is({ div: 1, p: 1 })) u = 0;
											...
											if (U == CKEDITOR.POSITION_IDENTICAL || U == CKEDITOR.POSITION_CONTAINS)
												...
												L = u.splitElement(M);
												u.moveToPosition(L, CKEDITOR.POSITION_AFTER_START);
												u = 1;
											}
										}
									}
								...
								if (u) q = x > 0;
									...
									if (
										!D.isBlock &&
										h.editor.config.autoParagraph !== false &&
										h.editor.activeEnterMode != CKEDITOR.ENTER_BR &&
										h.editor.editable().equals(u.blockLimit) &&
										!u.block &&
										(O =
											h.editor.activeEnterMode != CKEDITOR.ENTER_BR &&
											h.editor.config.autoParagraph !== false
												? h.editor.activeEnterMode == CKEDITOR.ENTER_DIV
													? 'div'
													: 'p'
												: false)
									) {
										...
										E.moveToPosition(O, CKEDITOR.POSITION_BEFORE_END);
									}
									...
									if (r && D.isElement) {
										...
										if (u) {
											...
											L = d(D.node, L);
											for (
												var M = [], U = L.length, X = 0, Z = void 0, $ = 0, aa = -1;
												X < U;
												X++
											) {
												Z = L[X];
						...
						if (Q)
							for (; (F = Q.pop());)
								CKEDITOR.env.needsBrFiller ? F.appendBogus() : F.append(K.document.createText(' '));
						for (; (F = h.mergeCandidates.pop());) F.mergeSiblings();
						K.moveToBookmark(R);
						if (!h.dontMoveCaret) {
							for (
								F = a(K.startContainer) && K.startContainer.getChild(K.startOffset - 1);
								F && a(F) && !F.is(n.$empty);
							) {
			...
			u = (function () {
				...
				return function (d) {
					...
					if (j.is('table')) {
						...
						d.moveToPosition(j, i ? CKEDITOR.POSITION_AFTER_START : CKEDITOR.POSITION_BEFORE_END);
					}
				};
			})();
	})();
	(function () {
		...
		function d() {
			...
			if (!s) {
				b.call(this);
				s = CKEDITOR.tools.setTimeout(b, 200, this);
			}
		}
		function b() {
			s = null;
		...
		function l(a, b, c) {
			...
			if (CKEDITOR.env.ie)
				...
				i.setStart(f.startContainer.$, f.startOffset);
				i.collapse(1);
				b.removeAllRanges();
				b.addRange(i);
			}
		...
		function h(a) {
			var b = CKEDITOR.dom.element.createFromHtml(
				'<div data-cke-hidden-sel="1" data-cke-temp="1" style="' +
					(CKEDITOR.env.ie ? 'display:none' : 'position:fixed;top:0;left:-1000px') +
					'">&nbsp;</div>',
				a.document
			);
			a.fire('lockSnapshot');
			...
			return function (c) {
				...
				if (b[d]) {
					...
					if (e.length == 1 && f.collapsed)
						if (
							(d = f[d < 38 ? 'getPreviousEditableNode' : 'getNextEditableNode']()) &&
			...
			for (var b = 0; b < a.length; b++) {
				...
				if (!c.collapsed) {
					...
					d &&
						...
						(f >= d.getLength() ? g.setStartAfter(d) : g.setStartBefore(d));
			...
			e.on('contentDom', function () {
				...
				f.attachListener(
					...
					function () {
						h &&
							CKEDITOR.env.webkit &&
							(h = e._.previousActive && e._.previousActive.equals(b.getActive()));
						e.unlockSelection(h);
						h = 0;
					},
					null,
				...
				if (CKEDITOR.env.ie || CKEDITOR.env.opera || m) {
					...
					p
						? f.attachListener(f, 'beforedeactivate', o, null, null, -1)
						: f.attachListener(e, 'selectionCheck', o, null, null, -1);
					f.attachListener(
						f,
							...
							g.on('mousedown', function (a) {
								function b(a) {
									...
									if (e) {
										...
										e.setEndPoint(
											i.compareEndPoints('StartToStart', c) < 0 ? 'EndToEnd' : 'StartToStart',
											c
										);
										e.select();
									}
		...
		CKEDITOR.dom.selection = function (a) {
			...
			if (CKEDITOR.env.webkit)
				((b.type == 'None' && this.document.getActive().equals(a)) ||
					(b.type == 'Caret' && b.anchorNode.nodeType == CKEDITOR.NODE_DOCUMENT)) &&
					l(a, b);
			...
			if (d)
				if (d.getRangeAt)
					f =
						(i = d.rangeCount && d.getRangeAt(0)) &&
						new CKEDITOR.dom.node(i.commonAncestorContainer);
				else {
					try {
						i = d.createRange();
					} catch (g) {}
					f = i && CKEDITOR.dom.element.get((i.item && i.item(0)) || i.parentElement());
				}
			if (
				!f ||
				...
				: function () {
						...
						if (!c || !c.rangeCount) b = CKEDITOR.SELECTION_NONE;
							...
							if (
								...
								A[d.childNodes[c.startOffset].nodeName.toLowerCase()]
							)
					...
					c = CKEDITOR.tools.tryThese(
						...
						function () {
							for (
								...
								e &&
								(!(c = a.getEnclosedNode()) ||
									!(c.type == CKEDITOR.NODE_ELEMENT && A[c.getName()] && (d = c)));
								e--
							)
					...
					if (this.isLocked) {
						...
						if (g) this.fake(g);
							...
							if (p) {
								...
								((g.startContainer.type == CKEDITOR.NODE_ELEMENT &&
									g.startContainer.getName() in b) ||
									(g.endContainer.type == CKEDITOR.NODE_ELEMENT &&
										g.endContainer.getName() in b)) &&
								...
								if (f) {
									...
									l =
										!(l && l.getText && l.getText().match(h)) &&
										(o || !b.hasPrevious() || (b.getPrevious().is && b.getPrevious().is('br')));
								...
								for (q = 0; q < a.length; q++) {
									...
									if (CKEDITOR.env.opera && g.collapsed && l.type == CKEDITOR.NODE_ELEMENT) {
										...
										if (
											(!h && !k && l.is(CKEDITOR.dtd.$removeEmpty)) ||
	...
	(function () {
					...
					if (o.equals(n)) {
						...
						if (p && j && CKEDITOR.dtd.$block[s])
							for (
								var t = o,
									...
									B = w.length,
									G = 0,
									t = B && new CKEDITOR.dom.range(t.getDocument());
								G < B;
								++G
							) {
								var A = w[G],
						...
						w = s
							? !k[s] || r
								...
								: p && !j
									? 0
									: (o.getPosition(n) | N) == N && (!g.childRule || g.childRule(o))
							: 1;
						if (w)
							if (
								(w = o.getParent()) &&
								((w.getDtd() || CKEDITOR.dtd.span)[f] || i) &&
								(!g.parentRule || g.parentRule(w))
							) {
								if (!q && (!s || !CKEDITOR.dtd.$removeEmpty[s] || (o.getPosition(n) | N) == N)) {
									q = b.clone();
									q.setStartBefore(o);
								}
								s = o.type;
		...
		function b(a) {
			...
			if (a.collapsed) {
				for (
					var e = new CKEDITOR.dom.elementPath(d.getParent(), a.root), g, i = 0, h;
					i < e.elements.length && (h = e.elements[i]);
					i++
				) {
		...
		function q(a, b) {
			...
			for (h in d)
				if (
					!((h == 'class' || this._.definition.fullMatch) && a.getAttribute(h) != n(h, d[h])) &&
					!(b && h.slice(0, 5) == 'data-')
				) {
					g = a.hasAttribute(h);
					a.removeAttribute(h);
			...
			if ((b = b && b.attributes))
				for (var d = 0; d < b.length; d++) {
					...
					if ((f = a.getAttribute(e))) {
						var g = b[d][1];
						(g === null || (g.test && g.test(f)) || (typeof g == 'string' && f == g)) &&
							a.removeAttribute(e);
					}
				}
			c || p(a);
		...
		CKEDITOR.style = function (a, b) {
			...
			this.type =
				a.type ||
				(J[c] ? CKEDITOR.STYLE_BLOCK : t[c] ? CKEDITOR.STYLE_OBJECT : CKEDITOR.STYLE_INLINE);
							...
							if (g)
								for (var k = 0; a[i + k + 1];) {
									for (
										var o = l.document,
											g = 0,
											h = o.getById(d[k].endNode),
											o = o.getById(d[k + 1].startNode);
										;
									) {
			...
			for (var g = 0; g < a.length; g++)
				if (CKEDITOR.env.webkit)
					...
					CKEDITOR.env.ie && CKEDITOR.env.version < 11
						? (a[g].$.styleSheet.cssText = a[g].$.styleSheet.cssText + f)
						: (a[g].$.innerHTML = a[g].$.innerHTML + f);
				}
		}
		var i = {};
		CKEDITOR.skin = {
	...
	(function () {
		if (CKEDITOR.env.webkit) CKEDITOR.env.hc = false;
			...
			var a = CKEDITOR.dom.element.createFromHtml(
				'<div style="width:0;height:0;position:absolute;left:-10000px;border:1px solid;border-color:red blue"></div>',
				CKEDITOR.document
			);
...
*/
				...
				return function (b, e) {
					for (var a = b.match(/[^#]./g), c = 0; 3 > c; c++) {
						var f = a,
							...
							d;
						d = parseInt(a[c], 16);
						d = ('0' + (0 > e ? 0 | (d * (1 + e)) : 0 | (d + (255 - d) * e)).toString(16)).slice(
							-2
						);
						f[h] = d;
					}
								...
								function () {
									...
									'horizontal' != a.labelLayout
										? f.push(
												...
												' role="radiogroup" aria-labelledby="' + c.labelId + '">',
												e.call(this, b, a),
												'</div>'
											)
										: ((d = {
				...
				var i = a.getParentEditor(),
					...
					n = CKEDITOR.tools.addFunction(function (f) {
						...
						CKEDITOR.env.ie6Compat &&
							((g = q.getChild(0).getFrameDocument()), g.on('mousemove', e), g.on('mouseup', d));
				...
				if (m == CKEDITOR.dialog._.currentTop) {
					...
					if (9 == c || c == CKEDITOR.SHIFT + 9)
						((c = c == CKEDITOR.SHIFT + 9),
							m._.tabBarMode
								? ((c = c ? t.call(m) : u.call(m)), m.selectPage(c), m._.tabs[c][0].focus())
								: e(c ? -1 : 1),
							(o = 1));
						...
						if (d) {
							for (var i in this._.contents[a.id]) {
								...
								'hbox' == l.type ||
									'vbox' == l.type ||
									!l.getInputElement() ||
									(l.requiredContent && !this._.editor.activeFilter.check(l.requiredContent)
										? l.disable()
										: (l.enable(), f++));
							}
				...
				if (!a.requiredContent || this._.editor.filter.check(a.requiredContent)) {
					...
					c = CKEDITOR.dom.element.createFromHtml(
						[
							...
							d,
							'"',
							e.gecko && 10900 <= e.version && !e.hc ? '' : ' href="javascript:void(0)"',
			...
			CKEDITOR.ui.dialog = {
				uiElement: function (a, b, c, e, d, g, f) {
					if (!(4 > arguments.length)) {
						...
						for (
							var m = h['class'] && h['class'].split ? h['class'].split(' ') : [], j = 0;
							j < m.length;
							j++
						)
							m[j] && (n[m[j]] = 1);
						...
						a.on('load', function () {
							...
							if (b) {
								var c =
									p.type in { checkbox: 1, ratio: 1 } && CKEDITOR.env.ie && CKEDITOR.env.version < 8
						...
						CKEDITOR.ui.dialog.uiElement.call(
							...
							function () {
								...
								for (var e = 0; e < c.length; e++) {
									...
									i
										? h.push('height:' + r(i[e]))
										: d && d.expand && h.push('height:' + Math.floor(100 / c.length) + '%');
		...
		function w(b) {
				...
				if (!c.getById('cke_pastebin')) {
					var o = b.getSelection(),
						...
						j = new CKEDITOR.dom.element(
							(CKEDITOR.env.webkit || i.is('body')) && !CKEDITOR.env.ie && !CKEDITOR.env.opera
								? 'body'
								: 'div',
							c
						);
					...
					f
						...
						: (CKEDITOR.env.webkit
								...
								: i
										.getAscendant(CKEDITOR.env.ie || CKEDITOR.env.opera ? 'body' : 'html', 1)
										.append(j),
							j.setStyles({
								position: 'absolute',
					...
					(f = j.getParent().isReadOnly())
						...
						: j.setStyle('ltr' == b.config.contentsLangDirection ? 'left' : 'right', '-1000px');
			...
			function m(a) {
				...
				var a = b.getSelection(),
					d = a.getRanges();
				return a.getType() == CKEDITOR.SELECTION_NONE || (1 == d.length && d[0].collapsed)
			...
			(function () {
				function a(d, c, i, e, f) {
					...
					b.addMenuItems &&
						b.addMenuItem(c, { label: k, command: c, group: 'clipboard', order: f });
				}
		...
		function z() {
			var b = new CKEDITOR.htmlParser.filter(),
				...
				h = CKEDITOR.tools.extend(
					{ area: 1, basefont: 1, embed: 1, iframe: 1, map: 1, object: 1, param: 1 },
		...
		function p(b, a) {
			b.enterMode == CKEDITOR.ENTER_BR
				? (a = a
						...
						.replace(/<\/?p>/g, ''))
				: b.enterMode == CKEDITOR.ENTER_DIV && (a = a.replace(/<(\/)?p>/g, '<$1div>'));
			return a;
		}
			...
			init: function (b) {
				...
				b.on(
					...
					function (c) {
						...
						'htmlifiedtext' == h
							? (f = y(b.config, f))
							: 'text' == d && 'html' == h && (f = A(b.config, f, a || (a = z(b))));
			...
			proto: {
				...
				onKeyDown: function (a, b) {
					...
					switch (d) {
						case 'next':
							for (
								var e = this._.focusIndex, d = this.element.getElementsByTag('a'), c;
								(c = d.getItem(++e));
							)
	...
	(function () {
		function q(a, b, c, i, f) {
			var f = CKEDITOR.tools.genKey(
				...
				h = g[f];
			h ||
				((h = g[f] = new CKEDITOR.ui.panel(b, i)),
				(h.element = c.append(CKEDITOR.dom.element.createFromHtml(h.render(a), b))),
							...
							var a = CKEDITOR.tools.bind(function () {
								...
								if (l.autoSize) {
									...
									CKEDITOR.env.ie &&
										CKEDITOR.env.quirks &&
										a > 0 &&
										(a = a + ((d.$.offsetWidth || 0) - (d.$.clientWidth || 0) + 3));
								...
								m
									? i < c && (e = g > c ? e + c : b.width > c ? e - a.left : e - a.right + b.width)
									: i < c && (e = g > c ? e - c : b.width > c ? e - a.right + b.width : e - a.left);
								c = a.top;
								b.height - a.top < f &&
									(k = c > f ? k - f : b.height > f ? k - a.bottom + b.height : k - a.top);
								if (CKEDITOR.env.ie) {
									...
									b.getComputedStyle('direction') == 'rtl' &&
										(e = CKEDITOR.env.ie8Compat
											? e - d.getDocument().getDocumentElement().$.scrollLeft * 2
	...
	(function () {
		...
		var h =
			'<span class="cke_menuitem"><a id="{id}" class="cke_menubutton cke_menubutton__{name} cke_menubutton_{state} {cls}" href="{href}" title="{title}" tabindex="-1"_cke_focus=1 hidefocus="true" role="{role}" aria-haspopup="{hasPopup}" aria-disabled="{disabled}" {ariaChecked}';
		if (CKEDITOR.env.opera || (CKEDITOR.env.gecko && CKEDITOR.env.mac))
			...
			proto: {
				...
				show: function (a, b, c, k) {
					...
					var b = b || ('rtl' == this.editor.lang.dir ? 2 : 1),
						...
						f = this._.element;
					if (!i) {
						i = this._.panel = new CKEDITOR.ui.floatPanel(
							this.editor,
						...
						this._.itemOverFn = CKEDITOR.tools.addFunction(function (a) {
							...
							this._.showSubTimeout = CKEDITOR.tools.setTimeout(
								this._.showSubMenu,
								j.config.menu_subMenuDelay || 400,
								this,
								[a]
							);
						}, this);
					...
					for (
						var d = j.elementPath(),
							d = [
								'<div class="cke_menu' +
									(d && d.direction() != j.lang.dir ? ' cke_mixed_dir_content' : '') +
									'" role="presentation">'
							],
							h = e.length,
							m = h && e[0].group,
						...
						var n = e[l];
						m != n.group &&
							(d.push('<div class="cke_menuseparator" role="separator"></div>'), (m = n.group));
						n.render(this, l, d);
					}
					d.push('</div>');
					f.setHtml(d.join(''));
					CKEDITOR.ui.fire('ready', this);
					this.parent
						? this.parent._.panel.showAsChild(i, this.id, a, b, c, k)
						: i.showBlock(this.id, a, b, c, k);
					j.fire('menuShow', [i]);
				},
			...
			if (!1 !== a.resize_enabled) {
				var c = null,
					...
					k = function () {
						...
						b.document &&
							(b.document.removeListener('mousemove', j), b.document.removeListener('mouseup', k));
					},
		...
		CKEDITOR.ui.button.prototype = {
			render: function (a, b) {
				...
				c = {
					...
					title: this.title,
					titleJs:
						c.gecko && 10900 <= c.version && !c.hc ? '' : (this.title || '').replace("'", ''),
	...
	(function () {
		function w(a) {
			function d() {
				for (
					var b = g(), e = CKEDITOR.tools.clone(a.config.toolbarGroups) || n(a), f = 0;
					f < e.length;
					f++
				) {
					var k = e[f];
									...
									for (var t = 0; t < s.length; t++) {
										...
										if (p)
											if (p.type == CKEDITOR.UI_SEPARATOR) j = e && p;
												...
												n
													? e ||
														(b.push('<span class="cke_toolgroup" role="presentation">'), (e = 1))
													: e && (b.push('</span>'), (e = 0));
												i = function (c) {
													c = c.render(a, b);
													q = l.items.push(c) - 1;
					...
					return function (d) {
						if ((r = a.editable()))
							if (
								...
								b.setStyle(
									d,
									t(('pin' == j ? p : l) + f + ('pin' == j ? 0 : 'left' == d ? s : -s))
								);
			...
			proto: {
				...
				render: function (a, b) {
					...
					d = {
						...
						cls: this.className || '',
						titleJs:
							d.gecko && 10900 <= d.version && !d.hc ? '' : (this.title || '').replace("'", ''),
						keydownFn: k,
						mousedownFn: m,
						focusFn: l,
						clickFn: e
				...
				createPanel: function (a) {
					if (!this._.panel) {
						...
						e.onHide = function (b) {
							...
							c.setState(
								c.modes && c.modes[a.mode] ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED
							);
							c._.on = 0;
		...
		function k(a) {
			...
			CKEDITOR.env.gecko &&
				...
				2e4 > CKEDITOR.env.version &&
					((c.innerHTML = c.innerHTML.replace(/^.*<\!-- cke-content-start --\>/, '')),
					setTimeout(function () {
			...
			CKEDITOR.env.ie &&
				...
				d.config.enterMode != CKEDITOR.ENTER_P &&
					this.attachListener(b, 'selectionchange', function () {
						var a = b.getBody(),
							c = d.getSelection(),
							e = c && c.getRanges()[0];
						e &&
					...
					if (d) (this.setHtml(a), b.fire('dataReady'));
						...
						e
							? (/<body[\s|>]/.test(a) || (a = '<body>' + a),
								...
								/<head[\s|>]/.test(a)
									? /<title[\s|>]/.test(a) || (a = a.replace(/<head[^>]*>/, '$&<title></title>'))
					...
					for (var l = new CKEDITOR.dom.walker(f), c; (c = l.next());)
						if (c.type == CKEDITOR.NODE_ELEMENT)
							if (!c.equals(j) && c.getDirection()) {
								...
								if (d)
									for (
										var g = b.data.dir == 'ltr' ? ['_rtl', ''] : ['', '_rtl'], h = 0;
										h < d.length;
										h++
									)
		...
		function s(e) {
			...
			for (
				...
				b &&
					...
					((c = new CKEDITOR.dom.walker(d)), (c.evaluator = i), (d.startContainer = c.next()));
				b &&
	...
	CKEDITOR.plugins.link = {
		...
		getEditorAnchors: function (b) {
			for (
				var a = b.editable(),
					d = a.isInline() && !b.plugins.divarea ? b.document : a,
					c = d.getElementsByTag('a'),
					a = [],
					f = 0,
					e;
				(e = c.getItem(f++));
			...
			if (this.fakeAnchor) {
				...
				for (f = 0; (e = d.getItem(f++));)
					(e = this.tryRestoreFakeAnchor(b, e)) &&
						a.push({ name: e.getAttribute('name'), id: e.getAttribute('id') });
			}
			return a;
		},
	...
	(function () {
		function E(c, j, e) {
			...
			for (
				var k = CKEDITOR.plugins.list.listToArray(j.root, e), g = [], h = 0;
				h < j.contents.length;
				h++
			) {
				var f = j.contents[h];
		...
		function A(c, j, e, b) {
			for (var k, g; (k = c[b ? 'getLast' : 'getFirst'](F));)
				((g = k.getDirection(1)) !== j.getDirection(1) && k.setAttribute('dir', g),
					k.remove(),
		...
		function y(c, j, e) {
			...
			for (; e.checkStartOfBlock() && e.checkEndOfBlock();) {
				...
				if (!b) break;
				b.is('li') &&
					((g = b.getParent()), b.equals(g.getLast(q)) && b.equals(g.getFirst(q)) && (b = g));
			...
			listToArray: function (c, j, e, b, k) {
				...
				for (var g = 0, h = c.getChildCount(); g < h; g++) {
					...
					f.type == CKEDITOR.NODE_ELEMENT &&
						...
						CKEDITOR.plugins.list.listToArray(f, j, e, b + 1);
						...
						for (var d = 0, m = f.getChildCount(), i; d < m; d++)
							((i = f.getChild(d)),
								i.type == CKEDITOR.NODE_ELEMENT && r[i.getName()]
									? CKEDITOR.plugins.list.listToArray(i, j, e, b + 1, a.grandparent)
									: a.contents.push(i));
					}
				}
				return e;
			...
			arrayToList: function (c, j, e, b, k) {
				...
				for (
					...
					if (o.indent == m) {
						...
						r[g.getName()]
							? ((i = o.element.clone(!1, !0)), n != g.getDirection(1) && i.setAttribute('dir', n))
						...
						for (g = 0; g < x; g++)
							if (((s = o.contents[g]), D(s) && 1 < x))
								...
								y &&
									l.setAttribute(
										'style',
										y.replace(/([^;])$/, '$1;') + (l.getAttribute('style') || '')
									);
								z && s.addClass(z);
								l = null;
								t && (i.append(t), (t = null));
								...
								w
									? (l || ((l = h.createElement(p)), i.append(l), a && l.setAttribute('dir', n)),
						...
						i.type == CKEDITOR.NODE_DOCUMENT_FRAGMENT &&
							...
							(CKEDITOR.env.needsBrFiller &&
								...
								n.remove(),
							(n = i.getLast(q)),
							(!n || !(n.type == CKEDITOR.NODE_ELEMENT && n.is(CKEDITOR.dtd.$block))) &&
					...
					for (c = f.getFirst(); c;) {
						if (
							...
							c.getName() in CKEDITOR.dtd.$listItem &&
								((e = c), (h = k = b = void 0), (b = e.getDirection())))
						) {
		...
		x.prototype = {
			exec: function (c) {
				...
				for (
					var j = e.createBookmarks(!0), k = [], h = {}, b = b.createIterator(), f = 0;
					(g = b.getNextRange()) && ++f;
				) {
							...
							for (
								...
								if (r[n.getName()] && i.contains(n)) {
									...
									(d = n.getCustomData('list_group_object'))
										...
										: ((d = { root: n, contents: [a] }),
											k.push(d),
											CKEDITOR.dom.element.setMarker(h, n, 'list_group_object', d));
									m = 1;
									break;
								}
							m ||
								...
								m.getCustomData('list_group_object_' + f)
									...
									: ((d = { root: m, contents: [a] }),
										CKEDITOR.dom.element.setMarker(h, m, 'list_group_object_' + f, d),
										k.push(d)));
						}
				}
				for (n = []; 0 < k.length;)
					if (((d = k.shift()), this.state == CKEDITOR.TRISTATE_OFF))
						if (r[d.root.getName()]) {
							...
							for (a = 0; a < f.contents.length; a++)
								if (
									((l = f.contents[a]),
									(l = l.getAscendant('li', !0)) && !l.getCustomData('list_item_processed'))
								)
									...
									((g = f.shift()),
										...
										'false' == g.getAttribute('contenteditable')
											...
											: (g.copyAttributes(p),
												d &&
													g.getDirection() &&
													(p.removeStyle('direction'), p.removeAttribute('dir')),
												g.moveChildren(p),
												g.remove()),
										p.appendTo(l));
	...
	(function () {
			...
			q(d, {
				...
				place: function () {
					...
					d.is(x) || (k.top > b.scroll.y - 15 && k.top < b.scroll.y + 5)
						...
						: d.is(y) || (k.top > b.pane.bottom - 5 && k.top < b.pane.bottom + 15)
							? ((k.top = a.inInlineMode
									? b.editable.height + b.editable.padding.top + b.editable.padding.bottom
									: b.pane.bottom - 1),
								this.look(y))
							: (a.inInlineMode && (k.top -= b.editable.top + b.editable.border.top), this.look(s));
					a.inInlineMode &&
						(k.top--, (k.top += b.editable.scroll.top), (k.left += b.editable.scroll.left));
		...
		function N(a, c, d) {
			...
			n.ie && a.enterMode == CKEDITOR.ENTER_BR
				...
				: ((f = ((f = K(a.element, !0)) && f.data('cke-enter-mode')) || a.enterMode),
					(f = new M(F[f], a.doc)),
					...
					return function (b) {
						...
						if (!W(a, b) && b && !b.equals(a.editable) && !b.contains(a.editable)) {
							...
							(!r(a, e, !c) || ((f = r(a, e, !c)) && m(f) && f.is(a.triggers)))
								...
								: ((f = J(a, b)),
									m(f) &&
										(r(a, f, !c) ? (b = r(a, f, !c)) && m(b) && b.is(a.triggers) && d(f) : d(f)));
						}
					};
				})()
			};
		...
		function Y(a) {
			...
			return !f && 0 <= c.size.top && 0 < d.y && d.y < c.size.top + e
				? ((a = a.inInlineMode || 0 === b.scroll.y ? x : s), new u([null, c, D, H, a]))
				: f && c.size.bottom <= b.pane.height && d.y > c.size.bottom - e && d.y < b.pane.height
					? ((a =
							a.inInlineMode || (c.size.bottom > b.pane.height - e && c.size.bottom < b.pane.height)
								? y
								: s),
						new u([c, null, Z, H, a]))
					: null;
				...
				(e.equals(a.editable[h ? 'getLast' : 'getFirst'](a.isRelevant))
					...
					(f = [null, e]
						...
						.concat([
							...
							H,
							i,
							e.equals(a.editable[h ? 'getLast' : 'getFirst'](a.isRelevant)) ? (h ? y : x) : s
						])));
		...
		function la(a, c, d, b) {
			for (
				var e = b, f = b, i = 0, h = !1, g = !1, j = a.view.pane.height, l = a.mouse;
				l.y + i < j && 0 < l.y - i;
			) {
				h || (h = c(e, b));
				g || (g = c(f, b));
		...
		var ca = (function () {
				return function (a, c, d) {
					...
					c &&
						v(a, f) &&
						(e.hide(), (f = new CKEDITOR.dom.element(b.$.elementFromPoint(d.x, d.y))), e.show());
					return !f ||
						!(f.type == CKEDITOR.NODE_ELEMENT && f.$) ||
						(n.ie && 9 > n.version && !a.boundary.equals(f) && !a.boundary.contains(f))
						? null
						: f;
				};
			})(),
							...
							'wysiwyg' == a.mode
								? j
									? (CKEDITOR.env.gecko && q(a),
										...
										(i = a.getSelection().getStartElement()) && i.scrollIntoView(!0))
									: d.$.scrollTo(k.x, k.y)
	...
	(function () {
		function h(a, d, f) {
			...
			b
				? f()
				: ((a = CKEDITOR.getUrl(a.config.pasteFromWordCleanupFile || d + 'filter/default.js')),
							...
							for (b = b.getNextSourceNode(!0, CKEDITOR.NODE_ELEMENT); b && !b.equals(j);)
								((d = b.getNextSourceNode(!1, CKEDITOR.NODE_ELEMENT)),
									!('img' == b.getName() && b.data('cke-realelement')) &&
										...
										(h.test(b.getName())
											? b.remove(1)
											: (b.removeAttributes(e), a.fire('removeFormatCleanup', b))),
									(b = d));
						}
						c.moveToBookmark(i);
					}
	...
	(function () {
		...
		var j = '',
			r = function () {
				...
				e[f] = CKEDITOR.tools.override(e[f], function (a) {
					return function (b) {
						...
						e &&
							...
							((this.contents = e.reset(c) || ''), (b.contents = e.reset(g) || ''));
				...
				b.document &&
					(b.elementMode != CKEDITOR.ELEMENT_MODE_INLINE || b.focusManager.hasFocus) &&
					g();
			};
		CKEDITOR.plugins.scayt = {
			...
			getUiTabs: function (a) {
				...
				for (var d = 0; 4 > d; d++)
					c[d] =
						'undefined' != typeof window.scayt && 'undefined' != typeof window.scayt.uiTags
			...
			init: function (a) {
				...
				c = function (b) {
					...
					CKEDITOR.env.opera || CKEDITOR.env.air
						...
						: f.setState(d.isScaytEnabled(a) ? CKEDITOR.TRISTATE_ON : CKEDITOR.TRISTATE_OFF);
				};
				...
				c.ui.addRichCombo('Styles', {
					...
					onRender: function () {
						c.on(
							...
							function (b) {
								for (
									var a = this.getValue(), b = b.data.path.elements, c = 0, d = b.length, g;
									c < d;
									c++
								) {
									g = b[c];
				...
				exec: function (d) {
					if (d.editable().hasFocus) {
						var e = d.getSelection(),
							b;
						if (
							(b = new CKEDITOR.dom.elementPath(e.getCommonAncestor(), e.root).contains(
								{ td: 1, th: 1 },
								1
							))
						) {
			...
			function d(a) {
				return CKEDITOR.tools.extend(a || {}, {
					...
					refresh: function (a, e) {
						this.setState(
							e.contains('table', 1) ? CKEDITOR.TRISTATE_OFF : CKEDITOR.TRISTATE_DISABLED
						);
					}
				});
			}
	...
	CKEDITOR.tools.buildTableMap = function (p) {
		for (var p = p.$.rows, o = -1, q = [], r = 0; r < p.length; r++) {
			...
			for (var k = -1, u = 0; u < p[r].cells.length; u++) {
				...
				for (k++; q[o][k];) k++;
				for (
					var m = isNaN(t.colSpan) ? 1 : t.colSpan, t = isNaN(t.rowSpan) ? 1 : t.rowSpan, v = 0;
					v < t;
					v++
				) {
					q[o + v] || (q[o + v] = []);
		...
		g.prototype = {
			...
			restoreImage: function (a) {
				...
				a.bookmarks
					...
					: CKEDITOR.env.ie &&
						((d = this.editor.document.getBody().$.createTextRange()), d.collapse(!0), d.select());
				this.locked = 0;
				this.index = a.index;
	...
	CKEDITOR.plugins.add('wsc', {
		...
		init: function (a) {
			...
			a.addCommand('checkspell', new CKEDITOR.dialogCommand('checkspell')).modes = {
				wysiwyg:
					!CKEDITOR.env.opera && !CKEDITOR.env.air && document.domain == window.location.hostname
			};
