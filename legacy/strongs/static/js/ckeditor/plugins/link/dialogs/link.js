...
CKEDITOR.dialog.add('link', function (m) {
	...
	function s(a) {
		...
		for (var b = 0; b < c.length; b++)
			((d = c[b].toLowerCase()),
				(e = a[d]),
				0 < b && g.push(','),
				g.push("'", e ? q(encodeURIComponent(a[d])) : '', "'"));
		g.push(')');
		return g.join('');
	}
	function t(a) {
	...
	var v = CKEDITOR.plugins.link,
		...
		H = function (a, g) {
			...
			if (!b.type)
				if ((d = c.match(A)))
					((b.type = 'anchor'), (b.anchor = {}), (b.anchor.name = b.anchor.id = d[1]));
				else if ((d = c.match(x))) {
					e = c.match(y);
					c = c.match(z);
					b.type = 'email';
	...
	return {
								...
								children: [
									...
									},
									{
										...
										validate: CKEDITOR.dialog.validate.inlineStyle(
											m.lang.common.invalidInlineStyle
										),
										setup: j,
										commit: l
									}
		...
		onShow: function () {
			var a = this.getParentEditor(),
				b = a.getSelection(),
				c = null;
			(c = v.getSelectedLink(a)) && c.hasAttribute('href')
