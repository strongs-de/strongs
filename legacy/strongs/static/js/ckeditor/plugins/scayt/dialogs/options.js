...
CKEDITOR.dialog.add('scaytcheck', function (j) {
	function w() {
		return 'undefined' != typeof document.forms['optionsbar_' + b]
			? document.forms['optionsbar_' + b].options
			: [];
	}
	function x(a, b) {
		if (a) {
		...
		B = {
			...
			onShow: function () {
				...
				if (!a.data || !a.data.scayt || !a.data.scayt_control)
					...
					var b = 0;
					s
						? a.data.scayt.getCaption(j.langCode || 'en', function (e) {
			...
			onCancel: function () {
				...
				a =
					'undefined' != typeof document.forms['languagesbar_' + b]
						? document.forms['languagesbar_' + b].scayt_lang
						: [];
				x(a, '');
			},
			contents: t
	...
	var A = function () {
			function a(a) {
				var c = f.getById('dic_name_' + b).getValue();
			...
			if (u) {
				...
				f.getById('dic_info_' + b).setHtml(h.dic_info);
			}
			if (1 == l[0])
				for (c in v)
					((d = 'label_' + v[c]),
		...
		y = function () {
			...
			1 == l[1] && ((g = f.getById('cke_option' + a.sLang)), x(g.$, a.sLang));
			u &&
