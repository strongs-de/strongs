		...
		E = (CKEDITOR.plugins.pastefromword = {
			utils: {
				...
				resolveList: function (a) {
					var c = a.attributes,
						b;
					if ((b = a.removeAnyChildWithName('cke:listbullet')) && b.length && (b = b[0]))
						return (
							(a.name = 'cke:li'),
							c.style &&
								(c.style =
			...
			filters: {
				...
				assembleList: function (a) {
					for (
						var c = a.children, b, f, d, e, h, j, a = [], g, m, i, l, k, p, n = 0;
						n < c.length;
						n++
					)
						if (((b = c[n]), 'cke:li' == b.name))
							if (
								((b.name = 'li'),
								(f = b.attributes),
								(i = (i = f['cke:listsymbol']) && i.match(/^(?:[(]?)([^\s]+?)([.)]?)$/)),
								(l = k = p = null),
								f['cke:ignored'])
							)
								c.splice(n--, 1);
				...
				stylesFilter: function (a, c) {
					return function (b, f) {
						...
						(b || '')
							.replace(/&quot;/g, '"')
							.replace(/\s*([^ :;]+)\s*:\s*([^;]+)\s*(?=;|$)/g, function (b, e, g) {
								e = e.toLowerCase();
							...
							if (CKEDITOR.env.gecko) {
								...
								a &&
									...
									(e.applyStyleFilter = function (a) {
										...
										b in c &&
											((b = c[b]), 'object' == typeof b && (b = b[d]), b && a.addStyle(b, !0));
									}));
							}
							return !1;
						},
							...
							if (p(a.parent)) delete a.name;
								...
								'font' == e.name
									...
									: ((d = d || ''),
										...
										b.face && ((d += 'font-family:' + b.face + ';'), delete b.face),
										b.size &&
											((d +=
												'font-size:' +
									...
									: CKEDITOR.env.gecko && c
										? ((c = CKEDITOR.htmlParser.fragment.fromHtml(c[0]).children[0]),
											(d =
												(d =
													(d = b.previous) &&
													d.value.match(/<v:imagedata[^>]*o:href=['"](.*?)['"]/)) && d[1]) &&
												(c.attributes.src = d),
											c)
										: !1;
							}
